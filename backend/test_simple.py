...
def test_abt_algorithm():
    ...
    # Add events
    print("Adding 5 events...")
    for i in range(5):
        abt.add_event(f"/test/file_{i}.txt")
    
    print(f"ABT after 5 events: {abt.calculate_abt():.2f}")
    print(f"Burst detected: {abt.is_burst_detected()}")
    
    # Add more events (burst)
    print("\nAdding 15 more events (simulating burst)...")
    for i in range(15):
        abt.add_event(f"/test/burst_{i}.txt")
    
    print(f"ABT after 20 events: {abt.calculate_abt():.2f}")
    print(f"Burst detected: {abt.is_burst_detected()}")
    
    print()

def analyze_test_files():
    """Analyze the test files for ransomware indicators"""
    print("Analyzing Test Files for Ransomware Indicators...")
    ...
    SUSPICIOUS_EXTENSIONS = {'.enc', '.locked', '.crypted', '.crypto', '.ransom'}
    
    test_files = [
        "/tmp/test_ransomguard/normal.txt",
        "/tmp/test_ransomguard/mixed.dat",
        "/tmp/test_ransomguard/encrypted.enc",
        "/tmp/test_ransomguard/normal.txt.encrypted",
        "/tmp/test_ransomguard/mixed.dat.encrypted"
    ]
    ...
    for file_path in test_files:
        if os.path.exists(file_path):
            ...
            if entropy > HIGH_ENTROPY_THRESHOLD:
                ...
                indicators.append("Medium entropy - Suspicious")
            
            if file_ext in SUSPICIOUS_EXTENSIONS:
                indicators.append("Suspicious extension")
            
            if indicators:
                print(f"  ⚠️  INDICATORS: {', '.join(indicators)}")
            else:
                print(f"  ✓ Normal file")
